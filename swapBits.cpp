...
{
	int *binary, indexk, *temp1, *temp2, index = 0;
	binary = (int*)malloc((p2 + n)*sizeof(int));
	temp1 = (int*)malloc(n*sizeof(int));
	temp2 = (int*)malloc(n*sizeof(int));
	unsigned int rev = 0, twop = 1;
	int len = 0;
	if (number <= 0 || p1<0 || p2<0 || n <= 0 || p1>32 || p2>32 || p1>p2)
		return 0;
	while (number > 0) {
	...
	for (index = len; index<(p2 + n); index++)
	{
		binary[index] = 0;
		len++;
	}
	indexk = 0;
	for (index = p1; index<(p1 + n); index++)
	{
		temp1[indexk] = binary[index];
		indexk++;
	}
	indexk = 0;
	for (index = p2; index<(p2 + n); index++)
	{
		temp2[indexk] = binary[index];
		indexk++;
	}
	indexk = 0;
	for (index = p1; index<(p1 + n); index++)
	{
		binary[index] = temp2[indexk];
		indexk++;
	}
	indexk = 0;
	for (index = p2; index<(p2 + n); index++)
	{
		binary[index] = temp1[indexk];
		indexk++;
	}
	if (len <= 32)
	{
